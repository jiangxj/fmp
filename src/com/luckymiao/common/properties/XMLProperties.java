...
public class XMLProperties {
	...
	public String getProperty(String name) {
		if(name != null && !"".equals(name) && name.startsWith("root.")){
	...
	private void getElementList(List<Element> nodeList, Element root,
			String elementName) {
	...
	private Element getElement(String path) {
		...
		for (int i = 0; i < propName.length; i++) {
			if (propName[i].contains(this.flag)) {
				...
				int elementIndex = 0;
				try {
					elementIndex = Integer.parseInt(propName[i]
							.split(this.flag)[1]);
	...
	public String getAttributeValue(String path, String name) {
		...
		value = value.trim();
		this.propertyCache.put(path, value);
		return value;
	}
